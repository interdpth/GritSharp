...
bool grit_xp_mem(GritRec *gr)
{
	...
	for (eint id = GRIT_ITEM_GFX; id<GRIT_ITEM_MAX; id++)
	{
		...
		if (item.procMode == GRIT_EXPORT)
		{
			...
			std::vector<unsigned char>* vectPnt;

			if(id== GRIT_ITEM_GFX)
			{
				vectPnt = &gr->parts.gfx;
			}
			else if (id == GRIT_ITEM_MAP)
			{
				vectPnt = &gr->parts.tilemap;
			}
			else if (id == GRIT_ITEM_METAMAP)
			{
				vectPnt = &gr->parts.metadata;
			}
			else if (id == GRIT_ITEM_PAL)
			{
				vectPnt = &gr->parts.pal;
			}
			vectPnt->clear();
			//vectPnt->resize(rec_size(item.pRec));
			for (int c = 0; c < rec_size(item.pRec); c++)
			{
				vectPnt->push_back((unsigned char)&item.pRec->data[c]);
			}
			//xp_data_mem((unsigned char*)&vectPnt->front(), item.pRec->data, rec_size(item.pRec));
		}
	}
