	...
	FREE_IMAGE_FORMAT *fifs, int fif_count)
{
	...
	if(count > 1)
	{
		for(ii=0; ii<count; ii++)
		{
			...
			strcat_s(filter,  1024, FreeImage_GetFIFExtensionList(fif));
			strcat_s(filter, 1024, ",");
		}
		filter[strlen(filter)-1]= '\0';

		strcpy_s(szFilter, 1024, "Supported image files |");
		char* next_token1 = NULL;

		token = strtok_s(filter, ",", &next_token1);
		while(token != NULL) 
		{
			sprintf_s(str, 1024, "*.%s;", token);
			strcat_s(szFilter, 1024, str);
			// get next token
	...
	for(ii=0; ii<count; ii++)
	{
		fif= (FREE_IMAGE_FORMAT)fiflist[ii];
		// Description
		sprintf_s(filter, 1024, "%s (%s)|",
			FreeImage_GetFIFDescription(fif),
			FreeImage_GetFIFExtensionList(fif));

		strcat_s(szFilter, 1024, filter);
		char* lol = NULL;
		// Extension(s)
		strcpy_s(filter, 1024, FreeImage_GetFIFExtensionList(fif));
		token = strtok_s(filter, ",", &lol);
		while(token != NULL) 
		{
			sprintf_s(str, 1024, "*.%s;", token);
			strcat_s(szFilter,  1024, str);
			// get next token
			token = strtok_s(NULL, ",", &lol);
		}
		szFilter[strlen(szFilter)-1] = '|';
	}
	strcat_s(szFilter, 1024, "|");
